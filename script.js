  ...
  delButton.addEventListener('click', () => {
    for (let z = 0; z < todoList.length; z++) {
      if (todoList[z].id == delButton.getAttribute('data-index')) {
        if (
          delButton.parentNode.parentNode.classList.contains('finishedTodo')
        ) {
          finishedTodo.removeChild(delButton.parentNode);
          if (
            finishedTasks.indexOf(delButton.getAttribute('data-index')) != -1
          ) {
            finishedTasks.splice(
              finishedTasks.indexOf(delButton.getAttribute('data-index')),
              1
            );
