...
// Прописываем действия по клику на кнопку Добавить
 ...
 function showMessages (){
    let showMessage = ''; 
     ...
     <img class='editButton' id='${i}' src="edit.png">
     <input type='checkbox' ${item.checked?'checked':''} id='item_${i}'>
     <label for='item_${i}' id='labelId_${i}'>${item.todo}<label>
     </li>
     `;
     });
 ...
 todo.addEventListener("click", event => { 
    if (event.target.getAttribute("class")== 'editButton'){  // если где произошло клик, соответсвует классу deleteButton  
       let editId = event.target.getAttribute("id");  // то берем значение id элемента который вызвало событие
       console.log(editId);
    //    document.querySelector(`#labelId_${editId}`).contentEditable = true;
    //    document.getElementById(`#labelId_${editId}`).select()
    let editable = document.querySelector(`#labelId_${editId}`).contentEditable = true;
    if(editable){
